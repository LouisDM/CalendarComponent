...
-(UIView *)weekView{
    if (!_weekView) {
        ...
        _weekView = [[UIView alloc]initWithFrame:CGRectMake(-1, 63+exHeight, self.view.bounds.size.width+2, 40)];
        NSArray *title = @[@"S",@"M",@"T",@"W",@"T",@"F",@"S"];
        for (int i =0 ; i < 7 ; i++) {
    ...
    if (!_dataArray) {
        ...
        for (int i = 0; i<MAXMOUTH; i++) {
            if (i == 0) {
                ...
                NSInteger oneLineCoune =( 7 - m.dayOfTheWeek + 1 ) % 7;
                if (oneLineCoune == 0) {
                    oneLineCoune = 7;
                }
